...
class RenderingButtonsSystem: public SystemBase
{
    ...
    public:
    RenderingButtonsSystem(EntityManager& em, State::Context context): m_context(context), m_rect({200,200}), m_text(m_context.fonts->get(fonts::ID::Aerial))
    {
    ...
    virtual void update(EntityManager& em) override   
    {
        for (auto e : EntityCapture<Button>(em))
        {
            ...
            if (m_rect.getGlobalBounds().contains({static_cast<float>(mouse_pos.x), static_cast<float>(mouse_pos.y)}))
            {
            ...
            if (m_context.mouse->is_held()) std::cout << "Hello\n";
        }
    }
